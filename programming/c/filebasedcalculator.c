...
double convstore(const char* str) { // convert and store, convenience store. this will change the number in the .txt file to a readable character for the program
    return strtod(str, NULL); // converts the string to a double to accommodate decimal points (for cases such as 8 / 3)
}
/*
* note: - extra integer-based code that was written and explained in this section before the potential use of decimal values was considered.
*       - if all doubles in the program are changed to int (line 42, line 86, line 111) and line 43 is removed, the following code will still function as intended when restored:
...
int main() {
    ...
    while (fgets(line_storage, sizeof(line_storage), in) != NULL) { // uses the buffer to read a line from the file
        ...
        double result = 0; // stores the solution of the current calculation
        ...
        while (tok != NULL) { // so long as there are valid tokens
            ...
            double number = convstore(newTok); // converts the string into a double to store in the number variable
